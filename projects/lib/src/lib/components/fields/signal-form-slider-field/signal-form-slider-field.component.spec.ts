...
import { SignalFormSliderFieldComponent } from './signal-form-slider-field.component';

interface TestModel {
  volume: number;
}

describe('SignalFormSliderFieldComponent', () => {
  let spectator: Spectator<SignalFormSliderFieldComponent<TestModel, 'volume'>>;
  let mockField: RuntimeSliderSignalField<TestModel, 'volume'>;
  let mockForm: SignalFormContainer<TestModel>;

  const createComponent = createComponentFactory<
    SignalFormSliderFieldComponent<TestModel, 'volume'>
  >({
    component: SignalFormSliderFieldComponent,
    shallow: true,
