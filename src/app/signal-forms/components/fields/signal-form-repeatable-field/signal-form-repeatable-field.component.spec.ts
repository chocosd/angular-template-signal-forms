...
import { SignalFormRepeatableFieldComponent } from './signal-form-repeatable-field.component';

interface TestModel {
  items: Array<{ name: string; value: number }>;
}

describe('SignalFormRepeatableFieldComponent', () => {
  let spectator: Spectator<SignalFormRepeatableFieldComponent<TestModel>>;
  let mockRepeatableForms: SignalFormContainer<unknown>[];
  let mockFields: SignalFormField<TestModel>[];

  const createComponent = createComponentFactory<
    SignalFormRepeatableFieldComponent<TestModel>
  >({
    component: SignalFormRepeatableFieldComponent,
    shallow: true,
